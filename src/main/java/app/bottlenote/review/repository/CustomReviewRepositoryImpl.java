...
public class CustomReviewRepositoryImpl implements CustomReviewRepository {
	...
	public Pair<Long, CursorResponse<ReviewExploreItem>> getStandardExplore(Long userId, List<String> keywords, Long cursor, Integer size) {
		...
		// GROUP_CONCAT 표현식
		StringExpression groupConcatImages = Expressions.stringTemplate(
				"GROUP_CONCAT(DISTINCT {0})",
				reviewImage.reviewImageInfo.imageUrl
		);

		StringExpression groupConcatTags = Expressions.stringTemplate(
				"GROUP_CONCAT(DISTINCT {0})",
				reviewTastingTag.tastingTag
		);
		...
		List<Tuple> results = queryFactory
				...
				.groupBy(
						review.id, review.content, review.reviewRating,
						review.createAt, review.lastModifyAt, review.isBest,
						user.id, user.nickName, user.imageUrl,
						alcohol.id, alcohol.korName
				)
		...
		for (Tuple tuple : results) {
			...
			UserInfo userInfo = UserInfo.of(
					...
					tuple.get(user.imageUrl)
			);

			// 태그 문자열을 List<String>으로 변환
			String tagStr = tuple.get(groupConcatTags);
			List<String> tags = tagStr != null ?
					new ArrayList<>(Arrays.asList(tagStr.split(","))) :
					new ArrayList<>();
			...
			items.add(new ReviewExploreItem(
					userInfo,
		...
		Long total = queryFactory
				...
				.leftJoin(reviewReply).on(review.id.eq(reviewReply.reviewId))
				.leftJoin(reviewTastingTag).on(review.id.eq(reviewTastingTag.review.id))
				.where(review.activeStatus.eq(ACTIVE), review.status.eq(PUBLIC), containsKeywordInAll(keywords))
				.fetchOne();
