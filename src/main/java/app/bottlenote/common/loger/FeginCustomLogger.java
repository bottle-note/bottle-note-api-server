...
public class FeginCustomLogger extends Logger {
    ...
    protected Response logAndRebufferResponse(String configKey, Level logLevel, Response response, long elapsedTime) throws IOException {
        ...
        if (response.reason() != null && logLevel.compareTo(Level.NONE) > 0)
            log.info("<--- HTTP/{} {} ({}ms, {}-byte body)", protocolVersion, response.status(), elapsedTime, response.body().length());
        else
            log.info("<--- HTTP/{} {} ({}ms)", protocolVersion, response.status(), elapsedTime);
