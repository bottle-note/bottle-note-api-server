 ...
 * 닉네임 생성기
 * <p>
 * 닉네임 생성기는 다음과 같은 방식으로 닉네임을 생성합니다. 1. 현재 시간을 기반으로 닉네임을 생성합니다. 2. 현재 시간의 연도의 마지막 두 자리를 더한 값을 구합니다.
 * -> 유니크함은 유지하지하면서 닉네임 길이를 줄일 수 있음. 3. 현재 시간의 월을 기반으로 알파벳을 매핑합니다. (1월: J, 2월: F, ... 11월: N, 12월:
 * D) -> 유니크함은 유지하지하면서 닉네임 길이를 줄일 수 있음. 4. 현재 시간의 일, 시, 분 값을 이어붙여 닉네임의 시간 부분을 생성 5. 랜덤으로 4개의 알파벳을
 * 생성합니다. - 랜덤 알파벳이 3개일 때와 4개일 때 생성되는 가능한 조합의 수 차이 - 3개일 때: 26 * 26 * 26 = 17,576 - 4개일 때: 26 * 26 *
 * 26 * 26 = 456,976 7. 생성된 랜덤 부분과 시간 부분을 이어붙여 최종 닉네임을 생성합니다.
 */
@Component
public class RandomLetterNicknameGenerator implements NicknameGenerator {
	...
	private RandomLetterNicknameGenerator() {
	...
	public String generateNickname() {
		String generatedNickname = generate();
		return makeUniqueNickname(generatedNickname);
	}

	public String makeUniqueNickname(String generatedNickname) {
		int attempts = 0;
		...
		while (oauthRepository.findByNickName(nickName).isPresent()) {
			...
			nickName = generate(); // 새로운 닉네임을 생성
			attempts++;
		}
		return nickName; // 유효한 닉네임을 반환
	}
