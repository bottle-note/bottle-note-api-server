...
class PicksCommandServiceTest {
  ...
  @InjectMocks private PicksCommandService picksCommandService;

  @Mock private UserFacade userFacade;

  @Mock private AlcoholFacade alcoholFacade;

  @Mock private PicksRepository picksRepository;

  @Mock private HistoryEventPublisher picksEventPublisher;
  ...
  class UpdatePicks {
    ...
    void case_2() {
      ...
      PicksUpdateRequest pickRequest = new PicksUpdateRequest(alcohol.getId(), PICK);
      Picks picks =
          Picks.builder().alcoholId(alcohol.getId()).userId(user.getId()).status(UNPICK).build();

      // when
      when(picksRepository.findByAlcoholIdAndUserId(alcohol.getId(), user.getId()))
          .thenReturn(Optional.ofNullable(picks));
    ...
    void case_3() {
      ...
      PicksUpdateRequest pickRequest = new PicksUpdateRequest(alcohol.getId(), PICK);
      Picks picks =
          Picks.builder().alcoholId(alcohol.getId()).userId(user.getId()).status(PICK).build();
