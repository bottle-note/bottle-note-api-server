...
class ReviewReplyIntegrationTest extends IntegrationTestSupport {
	...
	class create {
		...
		void test_1() throws Exception {
			...
			ReviewReplyRegisterRequest replyRegisterRequest = new ReviewReplyRegisterRequest("댓글 내용", null);
			final Long reviewId = 1L;

			MvcResult result = mockMvc.perform(post("/api/v1/review/reply/register/{reviewId}", reviewId)
			...
			ReviewReplyResponse reviewReplyResponse = mapper.convertValue(response.getData(), ReviewReplyResponse.class);

			assertEquals(ReviewReplyResultMessage.SUCCESS_REGISTER_REPLY, reviewReplyResponse.codeMessage());
		}
		...
		void test_2() throws Exception {
			...
			final Long reviewId = 1L;
			ReviewReply savedReply = reviewReplyRepository.save(ReviewReply.builder()
				.reviewId(reviewId)
			...
			ReviewReplyRegisterRequest replyRegisterRequest = new ReviewReplyRegisterRequest("대댓글 내용", savedReply.getId());
	...
	class read {
		...
		void test_2() throws Exception {
			...
			MvcResult result = mockMvc.perform(get("/api/v1/review/reply/{reviewId}/sub/{rootReplyId}", reviewId, savedReply.getId())
					.contentType(MediaType.APPLICATION_JSON)
					.content(mapper.writeValueAsString(replyRegisterRequest))
					.header("Authorization", "Bearer " + getToken())
					.with(csrf())
	...
	class delete {
		...
		void test_1() throws Exception {
			...
			mockMvc.perform(delete("/api/v1/review/reply/{reviewId}/{replyId}", reviewId, savedReply.getId())
					.contentType(MediaType.APPLICATION_JSON)
					.header("Authorization", "Bearer " + getToken())
					.with(csrf())
				)
