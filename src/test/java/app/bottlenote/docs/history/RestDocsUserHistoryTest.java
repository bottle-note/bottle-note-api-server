...
class RestDocsUserHistoryTest extends AbstractRestDocs {
	...
	protected Object initController() {
		return new UserHistoryController(userHistoryQueryService);
	}
	...
	void docs_1() throws Exception {
		...
		PageResponse<UserHistorySearchResponse> response = historyQueryFixture.getUserHistorySearchResponse();
		...
		mockMvc.perform(get("/api/v1/history/{targetUserId}", targetUserId)
			...
			.andDo(document(
				...
				responseFields(
					...
					fieldWithPath("errors").type(JsonFieldType.ARRAY).description("응답 성공 여부가 false일 경우 에러 메시지 목록 (없을 경우 빈 배열)"),

					fieldWithPath("data.totalCount").type(JsonFieldType.NUMBER).description("전체 히스토리 개수"),
					fieldWithPath("data.subscriptionDate").type(JsonFieldType.ARRAY).description("(구독 또는 가입 등) 관련 일시"),
					fieldWithPath("data.userHistories[]").type(JsonFieldType.ARRAY).description("히스토리 상세 목록"),

					fieldWithPath("data.userHistories[].historyId").type(JsonFieldType.NUMBER).description("히스토리 ID"),
					fieldWithPath("data.userHistories[].createdAt").type(JsonFieldType.ARRAY).description("히스토리 생성 시각(yyyy-MM-dd'T'HH:mm:ss)"),
					fieldWithPath("data.userHistories[].eventCategory").type(JsonFieldType.STRING).description("이벤트 카테고리"),
					fieldWithPath("data.userHistories[].eventType").type(JsonFieldType.STRING).description("이벤트 타입"),
					...
					fieldWithPath("data.userHistories[].content").type(JsonFieldType.STRING).optional().description("히스토리 컨텐츠"),
					fieldWithPath("data.userHistories[].dynamicMessage").type(JsonFieldType.OBJECT)
						.optional().description("동적으로 구성되는 메시지 내용(key-value)"),
					fieldWithPath("data.userHistories[].dynamicMessage.currentValue").type(JsonFieldType.STRING)
						.optional().description("동적 메시지의 현재 값"),
					// meta 정보들
