...
class RestDocsUserHistoryTest extends AbstractRestDocs {
	...
	private final AlcoholViewHistoryService alcoholViewHistoryService = mock(AlcoholViewHistoryService.class);
		...
		mockMvc.perform(get("/api/v1/history/{targetUserId}", targetUserId)
				...
				.andDo(document(
						...
						responseFields(
								...
								fieldWithPath("errors").description("응답 성공 여부가 false일 경우 에러 메시지 목록 (없을 경우 빈 배열)"),
								...
								fieldWithPath("data.userHistories[].createdAt").description("히스토리 생성 시각(yyyy-MM-dd'T'HH:mm:ss)"),
								...
								fieldWithPath("data.userHistories[].alcoholId").optional().description("술 ID (없을 경우 null)"),
								fieldWithPath("data.userHistories[].alcoholName").optional().description("술 이름"),
								fieldWithPath("data.userHistories[].imageUrl").optional().description("이미지 URL"),
								fieldWithPath("data.userHistories[].redirectUrl").optional().description("리다이렉트 URL"),
								fieldWithPath("data.userHistories[].content").optional().description("히스토리 컨텐츠"),
	...
	void docs_2() throws Exception {
		...
		var items = asList(historyQueryFixture.getFixtureViewHistoryItem(1L, "글렌피딕"),
		...
		// then
		mockMvc.perform(get("/api/v1/history/view/alcohols")
						.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())
				.andDo(document(
