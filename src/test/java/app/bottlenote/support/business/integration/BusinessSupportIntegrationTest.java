...
class BusinessSupportIntegrationTest extends IntegrationTestSupport {
	...
	@Autowired
	private BusinessSupportTestFactory businessFactory;
	@Autowired
	...
	void get_detail_success() throws Exception {
		...
		// when & then
		mockMvc.perform(get("/api/v1/business-support/{id}", support.getId())
						.header("Authorization", "Bearer " + getToken()))
				.andDo(print())
				.andExpect(status().isOk());
	}
	...
	void get_detail_fail_not_found() throws Exception {
		...
		// when & then
		mockMvc.perform(get("/api/v1/business-support/{id}", nonExistId)
						.header("Authorization", "Bearer " + getToken()))
				.andDo(print())
				.andExpect(status().isBadRequest());
	}
	...
	void delete_success() throws Exception {
		...
		// when & then
		mockMvc.perform(delete("/api/v1/business-support/{id}", support.getId())
						.with(csrf())
						.header("Authorization", "Bearer " + getToken()))
				.andDo(print())
				.andExpect(status().isOk());
