...
public class FakeProfanityFeginClient implements ProfanityFeginClient {
	...
	@Override
	public ResponseEntity<ProfanityResponse> requestVerificationProfanity(String text) {
		log.info("[Fake] requestVerificationProfanity 욕설 검사 요청 : {}", text);
		return ResponseEntity.ok(createProfanityResponse(text));
	}
