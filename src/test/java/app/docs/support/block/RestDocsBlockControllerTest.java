...
class RestDocsBlockControllerTest extends AbstractRestDocs {
    ...
    void createBlockTest() throws Exception {
        ...
        CollectionResponse<UserBlockItem> response = CollectionResponse.of(1L, List.of(
            new UserBlockItem(2L, "차단된사용자", LocalDateTime.now())
        ));
        ...
        mockMvc.perform(post("/api/v1/blocks")
            ...
            .andDo(document("block-create",
                requestFields(
                    fieldWithPath("blockedUserId").type(JsonFieldType.NUMBER).description("차단할 사용자 ID")
                ),
                responseFields(
                    fieldWithPath("success").type(JsonFieldType.BOOLEAN).description("성공 여부").ignored(),
                    fieldWithPath("code").type(JsonFieldType.STRING).description("응답 코드").ignored(),
                    fieldWithPath("errors").type(JsonFieldType.ARRAY).description("에러 목록").ignored(),
                    ...
                    fieldWithPath("meta.serverResponseTime").type(JsonFieldType.ARRAY).description("서버 응답 시간").ignored(),
                    fieldWithPath("meta.serverPathVersion").type(JsonFieldType.STRING).description("서버 경로 버전").ignored(),
                    fieldWithPath("data.totalCount").type(JsonFieldType.NUMBER).description("차단된 사용자 총 수"),
                    fieldWithPath("data.items").type(JsonFieldType.ARRAY).description("차단된 사용자 목록"),
                    fieldWithPath("data.items[].userId").type(JsonFieldType.NUMBER).description("차단된 사용자 ID"),
                    fieldWithPath("data.items[].userName").type(JsonFieldType.STRING).description("차단된 사용자 이름"),
                    fieldWithPath("data.items[].blockedAt").type(JsonFieldType.ARRAY).description("차단된 시각")
                )
            ));
    }
    ...
    void deleteBlockTest() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/blocks/{blockedUserId}", blockedUserId))
            ...
            .andDo(document("block-delete",
                pathParameters(
                    parameterWithName("blockedUserId").description("차단 해제할 사용자 ID")
                ),
                responseFields(
                    fieldWithPath("success").type(JsonFieldType.BOOLEAN).description("성공 여부").ignored(),
                    fieldWithPath("code").type(JsonFieldType.STRING).description("응답 코드").ignored(),
    ...
    void getBlockedUsersTest() throws Exception {
        // given
        Long currentUserId = 1L;
        CollectionResponse<UserBlockItem> response = CollectionResponse.of(2L, List.of(
    ...
    void checkBlockedTest() throws Exception {
        ...
        // then
        mockMvc.perform(get("/api/v1/blocks/check/{targetUserId}", targetUserId))
            .andExpect(status().isOk())
            .andDo(document("block-check",
                pathParameters(
                    parameterWithName("targetUserId").description("차단 여부를 확인할 사용자 ID")
                ),
                responseFields(
                    fieldWithPath("success").type(JsonFieldType.BOOLEAN).description("성공 여부").ignored(),
                    fieldWithPath("code").type(JsonFieldType.STRING).description("응답 코드").ignored(),
    ...
    void checkMutualBlockedTest() throws Exception {
        ...
        // then
        mockMvc.perform(get("/api/v1/blocks/mutual-check/{targetUserId}", targetUserId))
            .andExpect(status().isOk())
            .andDo(document("block-mutual-check",
                pathParameters(
    ...
    void getBlockedByCountTest() throws Exception {
        ...
        // then
        mockMvc.perform(get("/api/v1/blocks/stats/blocked-by-count"))
            .andExpect(status().isOk())
            .andDo(document("block-blocked-by-count",
                responseFields(
                    fieldWithPath("success").type(JsonFieldType.BOOLEAN).description("성공 여부").ignored(),
                    fieldWithPath("code").type(JsonFieldType.STRING).description("응답 코드").ignored(),
                    fieldWithPath("errors").type(JsonFieldType.ARRAY).description("에러 목록").ignored(),
    ...
    void getBlockingCountTest() throws Exception {
        ...
        // then
        mockMvc.perform(get("/api/v1/blocks/stats/blocking-count"))
            .andExpect(status().isOk())
            .andDo(document("block-blocking-count",
                responseFields(
                    fieldWithPath("success").type(JsonFieldType.BOOLEAN).description("성공 여부").ignored(),
                    fieldWithPath("code").type(JsonFieldType.STRING).description("응답 코드").ignored(),
                    fieldWithPath("errors").type(JsonFieldType.ARRAY).description("에러 목록").ignored(),
