...
public class RestReviewExploreControllerTest extends AbstractRestDocs {
	...
	void docs_standard_explore() throws Exception {
		...
		// then
		mockMvc.perform(get("/api/v1/reviews/explore/standard")
				...
				.andDo(
						document("reviews/explore/standard",
								queryParameters(
										parameterWithName("keywords").optional().description("검색어 목록 (작성자, 술 이름, 리뷰 내용, 테이스팅 태그에서 검색)"),
								...
								responseFields(
										...
										fieldWithPath("data.items[].userInfo.userProfileImage").description("사용자 프로필 이미지 URL"),
	...
	private List<ReviewExploreItem> createSampleReviewExploreItems() {
		...
		for (int i = 1; i <= 3; i++) {
			...
			List<String> images = List.of("https://example.com/image" + i + "_1.jpg", "https://example.com/image" + i + "_2.jpg");

			ReviewExploreItem item = new ReviewExploreItem(
					userInfo,
