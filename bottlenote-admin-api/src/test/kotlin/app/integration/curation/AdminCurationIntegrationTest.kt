...
class AdminCurationIntegrationTest : IntegrationTestSupport() {
	...
	inner class CreateCuration {
		...
		fun createSuccess() {
			// given
			val request =
				CurationHelper.createCurationCreateRequest(
					name = "새로운 큐레이션"
				)
	...
	inner class UpdateCuration {
		...
		fun updateSuccess() {
			...
			val curation = alcoholTestFactory.persistCurationKeyword()
			val request =
				CurationHelper.createCurationUpdateRequest(
					name = "수정된 큐레이션",
					description = "수정된 설명"
				)
	...
	inner class UpdateCurationStatus {
		...
		fun updateStatusSuccess() {
			...
			assertThat(
				...
				.bodyJson()
				.extractingPath("$.data.code")
				.isEqualTo("CURATION_STATUS_UPDATED")
		}
	...
	inner class UpdateCurationDisplayOrder {
		...
		fun updateDisplayOrderSuccess() {
			...
			assertThat(
				...
				.bodyJson()
				.extractingPath("$.data.code")
				.isEqualTo("CURATION_DISPLAY_ORDER_UPDATED")
		}
	}
	...
	inner class ManageCurationAlcohols {
		...
		fun addAlcoholsSuccess() {
			...
			val alcohol2 = alcoholTestFactory.persistAlcohol()
			val request =
				CurationHelper.createCurationAlcoholRequest(
					alcoholIds = setOf(alcohol1.id, alcohol2.id)
				)
			...
			assertThat(
				...
				.bodyJson()
				.extractingPath("$.data.code")
				.isEqualTo("CURATION_ALCOHOL_ADDED")
		}
		...
		fun removeAlcoholSuccess() {
			...
			assertThat(
				...
				.bodyJson()
				.extractingPath("$.data.code")
				.isEqualTo("CURATION_ALCOHOL_REMOVED")
		}
