...
class AdminBannerIntegrationTest : IntegrationTestSupport() {
	...
	inner class CreateBanner {
		...
		fun createSuccess() {
			// given
			val request =
				BannerHelper.createBannerCreateRequest(
					name = "새로운 배너"
				)
	...
	inner class UpdateBanner {
		...
		fun updateSuccess() {
			...
			val request =
				BannerHelper.createBannerUpdateRequest(
					name = "수정된 배너"
				)
	...
	inner class UpdateBannerStatus {
		...
		fun updateStatusSuccess() {
			...
			assertThat(
				...
				.bodyJson()
				.extractingPath("$.data.code")
				.isEqualTo("BANNER_STATUS_UPDATED")
		}
	...
	inner class UpdateBannerSortOrder {
		...
		fun updateSortOrderSuccess() {
			...
			assertThat(
				...
				.bodyJson()
				.extractingPath("$.data.code")
				.isEqualTo("BANNER_SORT_ORDER_UPDATED")
		}
	}
