...
object AdminMfdsMatchingApiDocs {
	...
	@Operation(
		...
		description = """
			기준 신고와 같은 product_identity_key_sha256 그룹의 현재 연결과 차이를 부작용 없이 보여 줍니다.
			숙성, 배치, 캐스크, 연도, 숫자 에디션, 캐스크 스트렝스, 도수 유형, 도수를 추가로 비교합니다.
			용량, 수입사, 통관일 차이는 같은 제품으로 봅니다. 식별 정보가 한쪽에만 있으면 확인 필요로 표시합니다.
			이미 다른 주류·증류소·지역이 있으면 충돌, 같은 3종이면 변경 불필요로 표시합니다.
			분류 우선순위는 충돌, 변경 불필요, 확인 필요, 적용 가능입니다. 확인 필요 사유는 여러 개일 수 있습니다.
			버전 표기, 변이 표기, 정제 검토 필요, 일반명, 제조국 불일치, 관리자 해제 이력도 확인 필요로 표시합니다.
			분류와 사유는 관리자가 고를 때 참고하는 안내이며 일괄 확정을 막지 않습니다.
	...
	@Operation(
		...
		description = """
			고른 신고에 주류·증류소·지역을 한 트랜잭션으로 반영합니다. 요청에는 적용할 주류·증류소·지역과 신고 ID 목록을 보냅니다.
			증류소와 지역을 생략하면 주류에 등록된 값을 사용합니다.
			비어 있는 연결은 채우고, 다른 값이 연결되어 있으면 덮어씁니다. 이미 같은 3종이 연결된 신고는 쓰지 않습니다.
			미리보기 분류나 같은 제품 그룹 소속은 검사하지 않으며, 어떤 신고를 고를지는 관리자가 판단합니다.
			응답은 반영된 신고의 확정 상태(applied)와 이미 같은 연결이라 바꾸지 않은 신고 ID(unchangedDeclarationIds)입니다.
			없는 신고, 중복 ID, 500건 초과, 없는 주류·증류소·지역이 있으면 아무것도 반영하지 않습니다.
			고른 신고는 ID 오름차순으로 잠급니다. 후보와 원문, 용량, 도수, 숙성, 배치 필드는 바꾸지 않습니다.
			각 신고의 감사 이력에 선택 근거와 기준 신고, 관리자를 남기고, 비워진 증류소·지역은 해제 이력으로 남깁니다.
			""",
		responses = [
			ApiResponse(
				responseCode = "200",
			...
			ApiResponse(
				responseCode = "400",
				description = "요청이 올바르지 않거나 선택한 주류·증류소·지역이 없습니다.",
				content = [Content(schema = Schema(ref = ERROR_SCHEMA))]
			),
			ApiResponse(
				responseCode = "404",
