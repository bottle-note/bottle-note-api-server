...
이 스크립트는 **500 VU가 각각 1회씩 요청**해 총 500개의 동시 요청으로 IP ban 요청 경로의 영향을 확인한다. 개발·운영 환경에서는 실행 전 대상과 부하 실행 승인을 반드시 확인한다.

```bash
# 고정 경량 공개 API 확인: 모든 응답이 200이어야 함
...
두 모드 모두 네트워크 실패(`status=0`), 4xx, 5xx가 0이고 p95가 1,000ms 미만, p99가 2,000ms 미만이어야 한다. fixed 모드는 모든 응답이 200이어야 한다. 상태 분포는 `status_network`, `status_2xx`, `status_3xx`, `status_4xx`, `status_5xx`, `status_other`의 6개 제한된 카운터로 출력된다.
