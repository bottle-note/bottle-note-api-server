...
public class PicksQuerySupporter {
  ...
  public Expression<Long> picksCountSubQuery(NumberPath<Long> userId) {
    return ExpressionUtils.as(
        select(picks.count())
            .from(picks)
            .where(picks.userId.eq(userId).and(picks.status.eq(PicksStatus.PICK))),
        "picksCount");
  }
  ...
  public BooleanExpression isPickedSubQuery(Long userId) {
    ...
    return Expressions.asBoolean(
        JPAExpressions.selectOne()
            .from(picks)
            .where(picks.alcoholId.eq(alcohol.id), picks.userId.eq(userId))
            .exists());
  }

  public BooleanExpression isPickedBothSubQuery(Long userId, Long targetUserId) {
    if (userId == null || targetUserId == null) return Expressions.FALSE;
  ...
  public Expression<Long> totalPicksCountSubQuery(NumberPath<Long> alcoholId) {
    return ExpressionUtils.as(
        JPAExpressions.select(picks.count()).from(picks).where(picks.alcoholId.eq(alcoholId)),
        "totalPicksCount");
  }
}
