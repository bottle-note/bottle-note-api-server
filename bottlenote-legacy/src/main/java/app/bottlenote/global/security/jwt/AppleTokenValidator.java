...
public class AppleTokenValidator implements TokenValidator {
  ...
  private final Cache<String, List<Map<String, String>>> publicKeyCache =
  ...
  public Claims validateAndGetClaims(String idToken, String expectedNonce) {
    ...
    try {
      header =
          new ObjectMapper()
              .readValue(new String(Base64.getDecoder().decode(headerOfIdToken)), Map.class);
  ...
  private PublicKey getPublicKey(Map<String, String> header) {
    ...
    Map<String, String> matchedKey =
        publicKeys.stream()
            .filter(
                key ->
                    key.get("kid").equals(header.get("kid"))
  ...
  private List<Map<String, String>> fetchApplePublicKeys() {
    ...
    Map<String, List<Map<String, String>>> keys =
        restTemplate.getForObject(APPLE_PUBLIC_KEYS_URL, Map.class);
