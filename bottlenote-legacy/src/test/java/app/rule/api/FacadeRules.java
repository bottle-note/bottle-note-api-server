...
public class FacadeRules extends AbstractRules {
  ...
  /** 퍼사드 인터페이스 명명 규칙을 검증합니다. 모든 퍼사드 인터페이스는 이름이 'Facade'로 끝나야 합니다. */
  @Test
  ...
  /** 퍼사드 구현체 명명 규칙을 검증합니다. 모든 퍼사드 구현체는 이름이 'Default'로 시작하고 'Facade'로 끝나야 합니다. */
  @Test
  public void 퍼사드_구현체_명명_규칙_검증() {
    ArchRule rule =
        classes()
            .that()
            .areAnnotatedWith(FacadeService.class)
            .should()
  ...
  /** 퍼사드 구현체 어노테이션 검증을 수행합니다. 'Facade'로 끝나는 구현체 클래스는 @FacadeService 어노테이션을 가져야 합니다. */
  @Test
  public void 퍼사드_구현체_어노테이션_검증() {
    ArchRule rule =
        classes()
            ...
            .areNotInterfaces()
            .should()
            .beAnnotatedWith(FacadeService.class)
            .because("퍼사드 구현체는 @FacadeService 어노테이션으로 명확히 식별되어야 합니다");

    rule.check(importedClasses);
  }

  /** 퍼사드 패키지 구조를 검증합니다. 모든 퍼사드 인터페이스는 도메인의 'facade' 패키지에 위치해야 합니다. */
  @Test
  ...
  /** 퍼사드 구현체 위치 검증을 수행합니다. 퍼사드 구현체는 'service' 패키지에 위치해야 합니다. */
  @Test
  ...
  public void 퍼사드_페이로드_명명_규칙_검증() {
    ArchRule rule =
        classes()
            ...
            .areNotNestedClasses()
            .should(
                new ArchCondition<>("have names ending with Info or Item") {
                  @Override
  ...
  /** 퍼사드와 컨트롤러 간의 의존성 검증을 수행합니다. 컨트롤러는 퍼사드 인터페이스에 의존할 수 있지만, 퍼사드 구현체에 직접 의존해서는 안 됩니다. */
  @Test
  ...
  /** 퍼사드 메서드 시그니처를 검증합니다. 퍼사드 메서드는 도메인 엔티티를 외부에 노출하지 않아야 합니다. */
  @Test
  public void 퍼사드_메서드_시그니처_검증() {
    ArchRule rule =
        methods()
            ...
            .should(
                new ArchCondition<>("not expose domain entities") {
                  @Override
                  public void check(JavaMethod method, ConditionEvents events) {
                    String returnTypeName = method.getReturnType().getName();

                    // 도메인 엔티티 확인 (패키지 경로에 domain이 포함되고 DTO, VO 등의 접미사가 없는 경우)
                    boolean isExposingEntity =
                        returnTypeName.contains(".domain.")
