...
class RestDocsBusinessSupportControllerTest extends AbstractRestDocs {
  ...
  void register_success() throws Exception {
    ...
    BusinessSupportUpsertRequest request =
        new BusinessSupportUpsertRequest(
            ...
            List.of(new BusinessImageItem(1L, "https://example.com/image1.jpg")));
    BusinessSupportResultResponse response =
        BusinessSupportResultResponse.response(REGISTER_SUCCESS, 1L);
        ...
        .andDo(
            document(
                ...
                requestFields(
                    ...
                    fieldWithPath("businessSupportType")
                        .type(JsonFieldType.STRING)
                        .description("문의 유형 (EVENT, ADVERTISEMENT, ETC)"),
    ...
    List<BusinessInfoResponse> infos =
        List.of(
            BusinessInfoResponse.of(
                2L, "두번째 문의 제목", "두번째 문의", LocalDateTime.now(), StatusType.WAITING),
            BusinessInfoResponse.of(
                1L, "첫번째 문의 제목", "첫번째 문의", LocalDateTime.now().minusDays(1), StatusType.SUCCESS));
    CollectionResponse<BusinessInfoResponse> collectionResponse =
        CollectionResponse.of(infos.size(), infos);
  ...
  void get_detail_success() throws Exception {
    ...
    Long userId = 1L;
    Long supportId = 1L;
    BusinessSupportDetailItem response =
        BusinessSupportDetailItem.builder()
            .id(supportId)
            .title("문의 상세 제목")
            .content("문의 상세 내용입니다.")
    ...
    mockMvc
        .perform(
            get("/api/v1/business-support/{id}", supportId).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andDo(
            document(
                "support/business/read-detail",
                pathParameters(parameterWithName("id").description("문의 아이디")),
                responseFields(
                    fieldWithPath("success").description("응답 성공 여부"),
                    fieldWithPath("code").description("응답 코드"),
  ...
  void modify_success() throws Exception {
    ...
    BusinessSupportUpsertRequest request =
        new BusinessSupportUpsertRequest(
            ...
            List.of(new BusinessImageItem(1L, "https://example.com/modified.jpg")));
    BusinessSupportResultResponse response =
        BusinessSupportResultResponse.response(MODIFY_SUCCESS, supportId);
    ...
    mockMvc
        .perform(
            patch("/api/v1/business-support/{id}", supportId)
                .with(csrf())
                .content(objectMapper.writeValueAsString(request))
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andDo(
            document(
                "support/business/modify",
                pathParameters(parameterWithName("id").description("수정할 문의 ID")),
                requestFields(
  ...
  void delete_success() throws Exception {
    ...
    Long supportId = 1L;
    BusinessSupportResultResponse response =
        BusinessSupportResultResponse.response(DELETE_SUCCESS, supportId);
    ...
    mockMvc
        .perform(
            delete("/api/v1/business-support/{id}", supportId)
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andDo(
            document(
                "support/business/delete",
                pathParameters(parameterWithName("id").description("삭제할 문의 ID")),
                responseFields(
                    fieldWithPath("success").description("응답 성공 여부"),
                    fieldWithPath("code").description("응답 코드"),
                    fieldWithPath("data.codeMessage").description("성공 메시지 코드"),
