...
class RestReviewReplyControllerTest extends AbstractRestDocs {
  ...
  class registry {
    ...
    void test_1() throws Exception {
      ...
      mockMvc
          ...
          .andDo(
              document(
                  "review/reply/register",
                  pathParameters(parameterWithName("reviewId").description("댓글을 작성 대상 리뷰의 식별자")),
                  requestFields(
  ...
  class select {
          ...
          .andDo(
              document(
                  ...
                  responseFields(
                      ...
                      fieldWithPath("data.reviewReplies[].imageUrl")
    ...
    void test_2() throws Exception {
      ...
      mockMvc
          .perform(
              get("/api/v1/review/reply/{reviewId}/sub/{rootReplyId}", reviewId, rootReplyId)
                  .contentType(MediaType.APPLICATION_JSON)
                  .param("cursor", "0")
                  .param("pageSize", "50"))
          .andExpect(status().isOk())
          .andDo(
              document(
                  ...
                  responseFields(
                      ...
                      fieldWithPath("data.reviewReplies[].parentReviewReplyId")
                          .description("상위 댓글 식별자"),
                      fieldWithPath("data.reviewReplies[].parentReviewReplyAuthor")
                          .description("상위 댓글 작성자 닉네임 (@ 기호는 제외)"),
                      fieldWithPath("data.reviewReplies[].reviewReplyId").description("댓글 자체 식별자"),
                      fieldWithPath("data.reviewReplies[].reviewReplyContent").description("댓글 내용"),
                      fieldWithPath("data.reviewReplies[].createAt").description("댓글 작성 일시"),
                      fieldWithPath("data.reviewReplies[].status").description("댓글 상태"),
