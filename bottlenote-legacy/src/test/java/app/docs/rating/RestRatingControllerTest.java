...
public class RestRatingControllerTest extends AbstractRestDocs {
  ...
  void test_2() throws Exception {
    ...
    RatingListFetchRequest request =
        RatingObjectFixture.ratingListFetchRequest("야마자키", AlcoholCategoryGroup.SINGLE_MALT, 1L);
    RatingListFetchResponse fetchList = RatingObjectFixture.ratingListFetchResponse();
    CursorPageable pageable =
        CursorPageable.builder().currentCursor(0L).cursor(0L).pageSize(10L).hasNext(false).build();
  ...
  void test_3() throws Exception {
    ...
    mockMvc
        .perform(
            RestDocumentationRequestBuilders.get("/api/v1/rating/{alcoholId}", alcoholId)
                .contentType(MediaType.APPLICATION_JSON)
                .with(csrf()))
        .andDo(print())
        .andExpect(status().isOk())
        .andDo(
            document(
                "rating/user-rating",
                pathParameters(parameterWithName("alcoholId").description("별점을 조회할 위스키의 식별자")),
                responseFields(
                    fieldWithPath("success").description("응답 성공 여부"),
                    fieldWithPath("code").description("응답 코드(http status code)"),
                    fieldWithPath("data.rating").description("등록된 별점"),
