...
- 스펙 목록 화면에서 `GET /admin/api/v1/curation-specs` 응답의 3개 spec이 표시된다.
- 스펙 상세 또는 카드 확장 영역에서 `requestSpec`, `responseSpec`, `hydratorKey`, `version`, `isActive`가 확인 가능하다.
- 큐레이션 목록 화면에서 Admin `GET /admin/api/v1/spec-based-curations`와 Product `GET /api/v2/curations`의 결과 차이를 구분해 확인할 수 있다.
...
- API surfaces:
  ...
  - Admin specs: `/admin/api/v1/curation-specs`, `/admin/api/v1/curation-specs/{specId}`
  - Admin spec-based curation: `/admin/api/v1/spec-based-curations`, `/admin/api/v1/spec-based-curations/{curationId}`
...
- Acceptance: development 환경에서 자동 로그인 후 `GET /admin/api/v1/curation-specs`가 3개 spec을 표시한다.
...
- Acceptance: 원본 `curation-new.html`의 동적 form UX를 가능한 유지하면서 Bottlenote Admin `POST /admin/api/v1/spec-based-curations` request body를 생성한다.
