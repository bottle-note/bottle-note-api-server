...
- 2026-08-23 Orca orchestration(run_6a1487308160)으로 Opus 워커 2기를 디스패치해 마지막 두 항목을 검증했고 **둘 다 worker_done으로 완료**했다. ① MySQL 방언: `ONLY_FULL_GROUP_BY` 위반 0건(파생 테이블의 SELECT가 전부 그룹 키 또는 집계), `SUM(CASE WHEN)`은 값 동일·타입만 DECIMAL, `VALUES()`는 8.0.20부터 deprecated이나 8.4에서도 제거되지 않아 배포에 지장 없음 — 다만 참조마다 경고가 올라오므로 로그량 주의. 행 별칭으로 바꾸면 H2가 지원하는지 먼저 확인해야 하고 아니면 SQL 실행 테스트가 깨진다. MySQL 실측은 도커 금지로 미검증이다. ② 참여도 커버리지: 비어 있던 경로를 지적받아 테스트 4건(증감 4종 분리, 값 불변 스킵, 0낙하, 재실행 하향 정정)을 추가했다. 다중 주류 GROUP BY 키와 삭제 리뷰에 달린 좋아요·댓글 배제는 여전히 미커버로 남는다.
- 2026-08-23 워커 1이 초기에 금지된 MySQL 도커 컨테이너를 띄워 즉시 제거하고 중단시켰다. 워커 프롬프트에 자원 제약을 넣지 않은 것이 원인이다 — 이후 정적 분석으로 전환해 완료했다.
