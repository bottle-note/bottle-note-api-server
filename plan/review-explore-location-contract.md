...
- 2026-08-20: Draft PR #713 첫 CI에서 rule-test와 Product integration이 실패했다. 원인은 `LocationInfoTest`의 `@Tag("unit")` 누락과 OpenAPI 계약 테스트가 nullable/composed schema의 `$ref`를 충분히 해석하지 못한 테스트 코드다. 기능 구현은 변경하지 않고 두 테스트 결함만 수정한 뒤 같은 PR CI를 재실행한다.
- 2026-08-20: 두 번째 CI에서 rule/unit/admin은 통과했으나 Product OpenAPI 계약 테스트가 재실패했다. CI artifact의 실제 schema는 `ReviewExploreResult.properties.items.items -> ReviewExploreItem.properties.locationInfo -> LocationInfo`이며 모두 정상 `$ref`다. 테스트가 array schema의 `items`를 `properties.items`로 잘못 탐색한 것이 원인으로 확정되어, 세 번째 최종 시도는 해당 traversal만 수정한다.
