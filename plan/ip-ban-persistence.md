...
- 2026-08-09: 후속 규칙 정리 완료 — request/response DTO와 projection enum을 규칙에 맞춰 rename·이동하고, JPA 감사 엔티티를 `IpBanAuditRecord`로 명확화했다. table/JPQL entity name·API payload·`IpBanEventType`은 유지했다. clean compile, 전체 `check_rule_test`와 Task 1~8 관련 smoke가 통과해 accesscontrol ArchUnit 잔여는 0건이다.
- 2026-08-09: Important 후속 보완 — 비활성 UNBANNED/EXPIRED 상태를 stateChangedAt/id keyset으로 끝까지 순회해 201번째 이후 stale Redis projection도 latest event version으로 제거한다. 기존 행 명령은 PESSIMISTIC_WRITE로 직렬화하고 신규 insert 경합은 DataIntegrityViolation/잠금 교착을 단발 재시도해 현재 상태와 BAN→EXTEND 감사 순서를 보장한다. signal의 ipBanId는 부모 존재와 normalized IP 일치를 검증한다. mono 단위 26/26, Product DB/Redis 통합 4/4·동시성 1/1·signal 4/4 통과.
- 2026-08-09: Important 후속 보완 — 비활성 재조정은 매분 최대 200건만 처리하고 `stateChangedAt/id` keyset cursor를 JDBC Quartz JobDataMap에 String으로 지속한다. `@PersistJobDataAfterExecution`과 `@DisallowConcurrentExecution`으로 multi-node 실행 간 cursor를 복원하며, 401건은 200→400→종료/reset으로 순환한다. mono 단위와 Product DB·Redis 통합을 갱신해 201번째가 두 주기 내 제거됨을 검증했다.
- 2026-08-09: Important 재검토 보완 — 비활성 projection 실패 시 실패 행에서 batch를 즉시 중단하고 마지막 성공 keyset만 유지해 다음 주기에 재시도한다. legacy orphan cleanup은 비활성 backlog와 독립된 20건 예산으로 version 없는 Redis 키만 조회한다.
