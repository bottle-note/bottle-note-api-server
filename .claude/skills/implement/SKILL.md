...
`git diff`와 `git status`로 요청 범위와 의도하지 않은 변경 여부를 확인한다. 이슈나 PR 본문에 진행 상태를 남기고 있다면 확인한 근거와 함께 갱신한다. 구현 중 확정된 설계 결정은 `adr/README.md` 규칙에 따라 ADR을 만들거나 갱신한다. 저장소에 계획서나 완료 도장을 만들지 않는다. 본문은 원형 ADR의 `Context(배경)` → `Decision(결정)` → `Status(상태)` → `Consequences(결과와 영향)` 순서로 작성하고 항목명은 `English(한글)`로 병기한다.
