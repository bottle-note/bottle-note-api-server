...
class AccessControlServiceTest {
  ...
  void setUp() {
    ...
    PathRateLimitRule reviewsRule = new PathRateLimitRule();
    reviewsRule.setPathPrefix("/api/v1/reviews");
    reviewsRule.setLimit(2);
    reviewsRule.setWindowSeconds(60);

    properties.setPathRules(List.of(authRule, exploreRule, reviewsRule));
  ...
  void evaluate_whenExplorePath_usesLongerPrefixRule() {
    for (int i = 0; i < 10; i++) {
      assertThat(service.evaluate("198.51.100.9", "/api/v1/reviews/explore").allowed()).isTrue();
    }
    // reviews 쓰기 한도(2)와 분리되어 explore 10까지 허용
    Decision denied = service.evaluate("198.51.100.9", "/api/v1/reviews/explore");
    assertThat(denied.type()).isEqualTo(Decision.Type.RATE_LIMITED);
  }
  ...
  void evaluate_whenManagementPathAndBanned_allows() {
    ...
    assertThat(service.evaluate("203.0.113.20", "/v1/access-control/ip-bans").allowed()).isTrue();
  }

  @Test
  @DisplayName("null IP는 unknown 한도를 적용한다")
  void evaluate_whenNullIp_appliesUnknownLimit() {
    assertThat(service.evaluate(null, "/api/v1/alcohols").allowed()).isTrue();
    assertThat(service.evaluate(null, "/api/v1/alcohols").allowed()).isTrue();
    Decision denied = service.evaluate(null, "/api/v1/alcohols");
    assertThat(denied.type()).isEqualTo(Decision.Type.RATE_LIMITED);
  }
