...
public interface JpaAlcoholsViewHistoryRepository
  ...
  @Query(
      ...
      SELECT new app.bottlenote.alcohols.dto.response.ViewHistoryItem(
          ...
          al.korName,
          al.engName,
          CAST(ROUND((SELECT COALESCE(AVG(r.ratingPoint.rating), 0.0) FROM rating r WHERE r.id.alcoholId = a.id.alcoholId AND r.ratingPoint.rating > 0.0) + 0.000000001D, 1) AS double),
          (SELECT COUNT(r) FROM rating r WHERE r.id.alcoholId = a.id.alcoholId AND r.ratingPoint.rating > 0.0),
          al.korCategory,
          al.engCategory,
          al.imageUrl,
          (CASE
