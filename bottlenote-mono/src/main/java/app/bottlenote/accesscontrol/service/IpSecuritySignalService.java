...
public class IpSecuritySignalService {
  ...
  @Transactional(readOnly = true)
  public Optional<IpSecuritySignalResponse> getDetail(Long signalId) {
    if (signalId == null) {
  ...
  @Transactional(readOnly = true)
  public List<IpSecuritySignalResponse> list(String rawIp, int max) {
  ...
  private IpSecuritySignalResponse toView(IpSecuritySignal signal) {
    return new IpSecuritySignalResponse(
        signal.getId(),
        signal.getIpBanId(),
