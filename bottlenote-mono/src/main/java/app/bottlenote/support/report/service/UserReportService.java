...
public class UserReportService {
  ...
  public UserReportResponse userReport(Long userId, UserReportRequest userReportRequest) {
    ...
    List<UserReports> userReportsList =
        userReportRepository.findByUserIdAndCreateAtAfter(user.getId(), today);
    ...
    log.info(
        "신고 처리 완료 :: 신고자 정보: {}({}) - 신고 대상자 정보: {}({})",
        user.getId(),
        user.getNickName(),
        reportUser.getId(),
        reportUser.getNickName());

    return UserReportResponse.of(
        SUCCESS, saveReport.getId(), reportUser.getId(), reportUser.getNickName());
  }

  /**
   * 해당 메소드는 유저 신고에 대한 비지니스 유효성 검사를 수행합 예를 들어 사용자 신고 대상이 이미 신고된 사용자인지, 일일 신고 횟수 제한을 초과하는지 등을 검사합니다.
   *
   * @param userReportRequest the user report request
  ...
  /**
   * 해당 메소드는 신고 내용에 대한 유효성 검사를 수행합니다. 이 검증 절차에는 신고 내용의 길이가 300자를 초과하는지, 부적절한 단어가 포함되어 있는지 등을 확인합니다.
