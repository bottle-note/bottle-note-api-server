...
- 코드와 git 히스토리에서 복원되지 않는 설계 결정은 `adr/YYYY.MM.DD 결정 한 문장.md`로 기록한다. 작성 규칙과 색인은 `adr/README.md`를 따르며, 결정이 바뀌면 기존 문서를 `대체됨`으로 표시하고 새 문서를 만든다. 문서가 없는 명확한 수정은 바로 진행할 수 있다.
