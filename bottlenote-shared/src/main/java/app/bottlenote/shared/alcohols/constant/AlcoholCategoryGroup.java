...
public enum AlcoholCategoryGroup {
  ...
  OTHER(
      "기타 위스키", Set.of("Single Pot Still", "Single Grain", "Spirit", "Tennessee", "Wheat", "Corn"));
  ...
  public static AlcoholCategoryGroup fromCategory(String categoryGroup) {
    ...
    return Arrays.stream(values())
        ...
        .findFirst()
        .orElseThrow(
            () -> new IllegalArgumentException("category group이 적절하지 않습니다. : " + categoryGroup));
  }

  public Boolean containsCategory(String category) {
    return Arrays.stream(values()).anyMatch(group -> group.categories.contains(category));
  }
}
