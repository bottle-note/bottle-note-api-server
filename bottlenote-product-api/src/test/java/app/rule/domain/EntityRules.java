...
public class EntityRules extends AbstractRules {
  ...
  public void 엔티티_명시적_이름_검증() {
    ArchRule rule =
        classes()
            ...
            .should(
                new ArchCondition<>("@Entity에 명시적인 이름이 선언되어야 함") {
                  @Override
                  public void check(JavaClass javaClass, ConditionEvents events) {
                    ...
                    String message =
                        String.format(
                            ...
                            javaClass.getName(), hasExplicitName ? "선언되어 있습니다" : "선언되어 있지 않습니다");
  ...
  /**
   * 모든 엔티티 클래스가 protected 접근 수준의 기본 생성자를 가지는지 검증합니다. Lombok의 @NoArgsConstructor는 SOURCE 리텐션이라 직접
   * 확인할 수 없으므로 생성자를 확인합니다.
   */
  @Test
  public void 엔티티_기본생성자_검증() {
    ArchRule rule =
        classes()
            ...
            .areAnnotatedWith(Entity.class)
            .should(
                new ArchCondition<>("protected 접근 수준의 기본 생성자를 가져야 함") {
                  @Override
                  public void check(JavaClass javaClass, ConditionEvents events) {
  ...
  public void 엔티티_toString_메서드_검증() {
    ArchRule rule =
        classes()
            ...
            .areAnnotatedWith(Entity.class)
            .and(
                new DescribedPredicate<>("양방향 연관관계가 있는 엔티티") {
                  @Override
            ...
            .should(
                new ArchCondition<>("안전한 toString 메서드를 구현해야 함") {
                  @Override
                  public void check(JavaClass javaClass, ConditionEvents events) {
                    // toString 메서드 확인
                    boolean hasToString =
                    ...
                    // @ToString 어노테이션 확인
                    boolean hasToStringAnnotation =
                        javaClass.getSourceCodeLocation().getSourceFileName() != null
                            && javaClass
  ...
  public void 엔티티_불변_필드_검증() {
    ArchRule rule =
        classes()
            ...
            .areAnnotatedWith(Entity.class)
            .should(
                new ArchCondition<>("ID 필드에 setter가 없어야 함") {
                  @Override
                  public void check(JavaClass javaClass, ConditionEvents events) {
                    // ID 필드 찾기
                    JavaField idField =
                            ...
                            .filter(
                                field ->
                                    field.isAnnotatedWith(Id.class)
                                        || field.isAnnotatedWith(EmbeddedId.class))
                            .findFirst()
                            .orElse(null);
                    ...
                    String message =
                        String.format(
                            ...
                            javaClass.getName(), !hasIdSetter ? "setter가 없습니다" : "setter가 있습니다");
  ...
  public void 비즈니스_메서드_캡슐화_검증() {
    ArchRule rule =
        classes()
            ...
            .areAnnotatedWith(Entity.class)
            .should(
                new ArchCondition<>("상태 변경이 명시적인 메서드로 캡슐화되어야 함") {
                  @Override
                  public void check(JavaClass javaClass, ConditionEvents events) {
                    // setter 메서드 개수
                    long setterCount =
                ...
                new ArchCondition<>("불변이어야 함") {
                  ...
                  public void check(JavaClass javaClass, ConditionEvents events) {
                    ...
                    String message =
                        String.format(
                            "임베디드 값 객체 '%s'는 %s",
                            javaClass.getName(), isImmutable ? "불변입니다" : "불변이 아닙니다");
  ...
  public void enum_타입_필드_처리_검증() {
    ArchRule rule =
        fields()
            ...
            .areAnnotatedWith(Entity.class)
            .and(
                new DescribedPredicate<>("is enum type") {
                  @Override
            ...
            .should()
            .beAnnotatedWith(Enumerated.class)
            .andShould(
                new ArchCondition<>("@Enumerated(EnumType.STRING)로 선언되어야 함") {
                  @Override
                    ...
                    if (!field.isAnnotatedWith(Enumerated.class)) {
                      events.add(
                          SimpleConditionEvent.violated(
                              field,
                              String.format(
                                  "필드 '%s'는 @Enumerated 어노테이션이 없습니다", field.getFullName())));
                      return;
                    }
