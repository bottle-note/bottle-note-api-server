...
class AlcoholReferenceControllerRestDocsTest extends AbstractRestDocs {
  ...
  void searchCurationKeywords() throws Exception {
    ...
    resultActions
        ...
        .andDo(
            document(
                "curation-keywords-search",
                queryParameters(
                    parameterWithName("keyword").description("큐레이션 키워드 이름 검색 (부분 일치)").optional(),
                    parameterWithName("alcoholId")
                        .description(
                            "위스키 식별자로 검색 - 해당 ID의 위스키가 포함된 큐레이션 조회 (예: alcoholId=123 검색 시 ID가 123인 위스키가 포함된 모든 큐레이션 반환)")
                        .optional(),
  ...
  void getCurationAlcohols() throws Exception {
    ...
    CursorResponse<AlcoholsSearchItem> response = CursorResponse.of(List.of(item), pageable);

    when(alcoholReferenceService.getCurationAlcohols(any(), any(), any())).thenReturn(response);
        ...
        .andDo(
            document(
                ...
                responseFields(
                    ...
                    fieldWithPath("data.items[].korCategoryName")
                        .type(STRING)
                        .description("카테고리 한글명"),
                    fieldWithPath("data.items[].engCategoryName")
                        .type(STRING)
