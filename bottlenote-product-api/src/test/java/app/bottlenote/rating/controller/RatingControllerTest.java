...
class RatingControllerTest {
  ...
  @MockitoBean private RatingQueryService queryService;
  @MockitoBean private RatingCommandService commandService;
  ...
  void setup() {
    ...
    rating =
        Rating.builder().id(RatingId.is(userId, alcoholId)).ratingPoint(RatingPoint.of(5)).build();
  }
  ...
  void test_1() throws Exception {
    ...
    RatingRegisterResponse response =
        RatingRegisterResponse.success(rating.getRatingPoint().getRating());

    // when
    when(SecurityContextUtil.getUserIdByContext()).thenReturn(Optional.of(userId));
    when(commandService.register(anyLong(), anyLong(), any(RatingPoint.class)))
        .thenReturn(response);
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.data.message").value(RatingRegisterResponse.Message.SUCCESS.getMessage()));
  }
  ...
  void test_3() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.errors[?(@.code == 'RATING_REQUIRED')].status")
                .value(ratingError.status().name()))
        .andExpect(
            jsonPath("$.errors[?(@.code == 'RATING_REQUIRED')].message")
                .value(ratingError.message()))
        .andExpect(
            jsonPath("$.errors[?(@.code == 'ALCOHOL_ID_REQUIRED')].status")
                .value(alcoholIdError.status().name()))
        .andExpect(
            jsonPath("$.errors[?(@.code == 'ALCOHOL_ID_REQUIRED')].message")
                .value(alcoholIdError.message()));
  }
}
