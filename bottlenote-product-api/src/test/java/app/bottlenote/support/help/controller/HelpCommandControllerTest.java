...
class HelpCommandControllerTest {
  ...
  @MockitoBean private HelpService helpService;
  ...
  void test_fail_when_unauthorized_user() throws Exception {
    ...
    when(helpService.deleteHelp(1L, 1L)).thenReturn(successDeleteResponse);

    // then
    mockMvc
        .perform(
            delete("/api/v1/help/{helpId}", 1L)
  ...
  class HelpRegisterControllerTest {
    ...
    void register_help_test() throws Exception {
      ...
      // when
      when(helpService.registerHelp(helpUpsertRequest, 1L)).thenReturn(successRegisterResponse);
  ...
  class HelpUpdateControllerTest {
    ...
    void update_help_test() throws Exception {
      ...
      // when
      when(helpService.modifyHelp(helpUpsertRequest, 1L, 1L)).thenReturn(successModifyResponse);

      // then
      mockMvc
          .perform(
              patch("/api/v1/help/{helpId}", 1L)
                  .contentType(MediaType.APPLICATION_JSON)
                  .content(mapper.writeValueAsString(helpUpsertRequest))
                  .with(csrf()))
          .andExpect(status().isOk())
    ...
    void test_fail_when_unauthorized_user() throws Exception {
      ...
      when(helpService.registerHelp(helpUpsertRequest, 1L)).thenReturn(successModifyResponse);
  ...
  class HelpDeleteControllerTest {
    ...
    void test_fail_when_help_is_not_exist() throws Exception {
      ...
      // when
      when(helpService.deleteHelp(1L, 1L)).thenThrow(new HelpException(HELP_NOT_FOUND));
    ...
    void test_fail_when_user_is_not_owner() throws Exception {
      ...
      // when
      when(helpService.deleteHelp(1L, 1L)).thenThrow(new HelpException(HELP_NOT_AUTHORIZED));
