...
class UserHistoryListenerTest {
  ...
  void replyActivity_preservesHistory() {
    historyListener.handleReviewReplyActivity(new ReviewReplyActivityEvent(10L, 20L, 1L, 2L, 30L, 3L, "댓글"));

    assertThat(userHistoryRepository.findAll()).singleElement().satisfies(history -> {
      ...
      assertThat(history.getRedirectUrl()).isEqualTo(RedirectUrlType.REVIEW.getUrl() + "/10");
    });
  }
  ...
  void likeActivity_preservesAllRequests() {
    historyListener.handleReviewLikeActivity(new ReviewLikeActivityEvent(30L, 10L, 20L, 1L, 2L, "리뷰", true));
    historyListener.handleReviewLikeActivity(new ReviewLikeActivityEvent(30L, 10L, 20L, 1L, 2L, "리뷰", false));

    assertThat(userHistoryRepository.findAll()).hasSize(2).allSatisfy(history -> {
      assertThat(history.getUserId()).isEqualTo(2L);
      assertThat(history.getAlcoholId()).isEqualTo(20L);
      assertThat(history.getEventCategory()).isEqualTo(EventCategory.REVIEW);
