...
class RestPopularControllerTest extends AbstractRestDocs {
  ...
  class WeeklyPopularDocs {

    @Test
    @DisplayName("주간 인기 술 리스트를 조회할 수 있다")
    void docs_getPopularOfWeek() throws Exception {
      // given
      ...
      mockMvc
          ...
          .andDo(
              document(
                  "alcohols/populars/week",
                  queryParameters(parameterWithName("top").description("조회할 주간 인기 술 목록 사이즈(갯수)")),
                  responseFields(
  ...
  @Nested
  @DisplayName("계절 인기 API")
  class SeasonalPopularDocs {

    @Test
    @DisplayName("봄에 인기 있는 술 리스트를 조회할 수 있다")
    void docs_getSpringItems() throws Exception {
      // given
      List<PopularItem> springPopulars =
          List.of(
              getFixturePopulars(1L, "글렌피딕", "Glenfiddich"),
              getFixturePopulars(2L, "맥캘란", "Macallan"),
              getFixturePopulars(3L, "글렌리벳", "Glenlivet"),
      ...
      // when & then
      mockMvc
          .perform(MockMvcRequestBuilders.get("/api/v1/popular/spring"))
          .andExpect(status().isOk())
          .andDo(
              document(
                  "alcohols/populars/spring",
                  responseFields(
                      fieldWithPath("success").description("응답 성공 여부"),
                      fieldWithPath("code").description("응답 코드(http status code)"),
