...
class RestReviewControllerDocsTest extends AbstractRestDocs {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/v1/reviews")
                ...
                .content(
                    objectMapper.writeValueAsString(ReviewObjectFixture.getReviewCreateRequest()))
                .with(csrf()))
        ...
        .andDo(
            document(
                ...
                requestFields(
                    ...
                    fieldWithPath("locationInfo.locationName")
                        .type(STRING)
                        .description("상호 명")
                        .optional(),
                    fieldWithPath("locationInfo.zipCode")
                        .type(STRING)
                        .description("우편번호")
                        .optional(),
                    fieldWithPath("locationInfo.address").type(STRING).description("주소").optional(),
                    fieldWithPath("locationInfo.detailAddress")
                        .type(STRING)
                        .description("상세 주소")
                        .optional(),
                    fieldWithPath("locationInfo.category")
                        .type(STRING)
                        .description("카테고리")
                        .optional(),
                    fieldWithPath("locationInfo.mapUrl")
                        .type(STRING)
                        .description("지도 URL")
                        .optional(),
  ...
  void review_read_test() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.get("/api/v1/reviews/1")
        ...
        .andDo(
            document(
                ...
                responseFields(
                    ...
                    fieldWithPath("data.reviewList[].userInfo.userProfileImage")
                    ...
                    fieldWithPath("data.reviewList[].locationInfo.zipCode")
                        .description("리뷰 장소 우편번호"),
                    fieldWithPath("data.reviewList[].locationInfo.address").description("리뷰 장소 주소"),
                    fieldWithPath("data.reviewList[].locationInfo.detailAddress")
                        .description("리뷰 장소 상세 주소"),
                    fieldWithPath("data.reviewList[].locationInfo.category")
                        .description("리뷰 장소 카테고리"),
                    fieldWithPath("data.reviewList[].locationInfo.mapUrl")
                        .description("리뷰 장소 지도 URL"),
                    fieldWithPath("data.reviewList[].locationInfo.latitude")
                        .description("리뷰 장소 위도"),
                    fieldWithPath("data.reviewList[].locationInfo.longitude")
                        .description("리뷰 장소 경도"),
                    fieldWithPath("data.reviewList[].status")
                    ...
                    fieldWithPath("data.reviewList[].createAt")
  ...
  void my_review_read_test() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.get("/api/v1/reviews/me/1")
                .param("sortType", "POPULAR")
                .param("sortOrder", "DESC")
                .param("cursor", "0")
                .param("pageSize", "2"))
  ...
  void review_detail_read_test() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.get("/api/v1/reviews/detail/1")
                .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andDo(
            document(
                "review/review-detail-read",
                responseFields(
                    fieldWithPath("success").description("요청 성공 여부").type(JsonFieldType.BOOLEAN),
                    fieldWithPath("code").description("응답 코드").type(JsonFieldType.NUMBER),

                    // Alcohol Item
                    fieldWithPath("data.alcoholInfo.alcoholId")
                        .description("술 ID")
                        .type(JsonFieldType.NUMBER),
                    fieldWithPath("data.alcoholInfo.korName")
                        .description("술의 한국어 이름")
                        .type(JsonFieldType.STRING),
                    fieldWithPath("data.alcoholInfo.engName")
                        .description("술의 영어 이름")
                        .type(JsonFieldType.STRING),
                    fieldWithPath("data.alcoholInfo.korCategoryName")
                        .description("술 카테고리의 한국어 이름")
                        .type(JsonFieldType.STRING),
                    fieldWithPath("data.alcoholInfo.engCategoryName")
                        .description("술 카테고리의 영어 이름")
                        .type(JsonFieldType.STRING),
                    fieldWithPath("data.alcoholInfo.imageUrl")
                        .description("술 이미지 URL")
                        .type(JsonFieldType.STRING),
                    fieldWithPath("data.alcoholInfo.isPicked")
                        .description("선택 여부")
                        .type(JsonFieldType.BOOLEAN),

                    // Review Response
                    fieldWithPath("data.reviewInfo.reviewId")
                        .description("리뷰 ID")
                        .type(JsonFieldType.NUMBER),
                    fieldWithPath("data.reviewInfo.reviewContent")
                        .description("리뷰 내용")
                        .type(JsonFieldType.STRING),
                    fieldWithPath("data.reviewInfo.price")
                        .description("가격")
                        .type(JsonFieldType.NUMBER),
                    fieldWithPath("data.reviewInfo.sizeType")
                        .description("사이즈 타입")
                        .type(JsonFieldType.STRING),
                    fieldWithPath("data.reviewInfo.likeCount")
                        .description("좋아요 개수")
                        .type(JsonFieldType.NUMBER),
                    fieldWithPath("data.reviewInfo.replyCount")
                        .description("댓글 개수")
                        .type(JsonFieldType.NUMBER),
                    fieldWithPath("data.reviewInfo.reviewImageUrl")
                        .description("리뷰 이미지 URL")
                        .type(JsonFieldType.STRING),
                    fieldWithPath("data.reviewInfo.totalImageCount").description("리뷰 이미지 총 개수"),

                    // User Item
                    fieldWithPath("data.reviewInfo.userInfo")
                        .description("사용자 정보")
                        .type(JsonFieldType.OBJECT),
                    fieldWithPath("data.reviewInfo.userInfo.userId")
                        .description("사용자 ID")
                        .type(JsonFieldType.NUMBER),
                    fieldWithPath("data.reviewInfo.userInfo.nickName")
                        .description("닉네임")
                        .type(JsonFieldType.STRING),
                    fieldWithPath("data.reviewInfo.userInfo.userProfileImage")
                        .description("사용자 프로필 이미지 URL")
                        .type(JsonFieldType.STRING),
                    fieldWithPath("data.reviewInfo.rating")
                        .description("평점")
                        .type(JsonFieldType.NUMBER),
                    fieldWithPath("data.reviewInfo.viewCount").description("조회수"),

                    // Location Item
                    fieldWithPath("data.reviewInfo.locationInfo")
                        .description("위치 정보")
                        .type(JsonFieldType.OBJECT)
                        .optional(),
                    fieldWithPath("data.reviewInfo.locationInfo.name")
                        .description("상호 명")
                        .type(JsonFieldType.STRING)
                        .optional(),
                    fieldWithPath("data.reviewInfo.locationInfo.zipCode")
                        .description("우편번호")
                        .type(JsonFieldType.STRING)
                        .optional(),
                    fieldWithPath("data.reviewInfo.locationInfo.address")
                        .description("주소")
                        .type(JsonFieldType.STRING)
                        .optional(),
                    fieldWithPath("data.reviewInfo.locationInfo.detailAddress")
                        .description("상세주소")
                        .type(JsonFieldType.STRING)
                        .optional(),
                    fieldWithPath("data.reviewInfo.locationInfo.category")
                        .description("카테고리")
                        .type(JsonFieldType.STRING)
                        .optional(),
                    fieldWithPath("data.reviewInfo.locationInfo.mapUrl")
                        .description("지도 URL")
                        .type(JsonFieldType.STRING)
                        .optional(),
                    fieldWithPath("data.reviewInfo.locationInfo.latitude")
                        .description("위도")
                        .type(JsonFieldType.STRING)
                        .optional(),
                    fieldWithPath("data.reviewInfo.locationInfo.longitude")
                        .description("경도")
                        .type(JsonFieldType.STRING)
                        .optional(),
                    fieldWithPath("data.reviewInfo.status")
                        .description("리뷰 상태")
                        .type(JsonFieldType.STRING),
                    fieldWithPath("data.reviewInfo.isMyReview")
                        .description("내 리뷰 여부")
                        .type(JsonFieldType.BOOLEAN),
                    fieldWithPath("data.reviewInfo.isLikedByMe")
                        .description("내가 좋아요를 눌렀는지 여부")
                        .type(JsonFieldType.BOOLEAN),
                    fieldWithPath("data.reviewInfo.hasReplyByMe")
                        .description("내가 댓글을 달았는지 여부")
                        .type(JsonFieldType.BOOLEAN),
                    fieldWithPath("data.reviewInfo.isBestReview")
                    ...
                    // Review Image List
                    fieldWithPath("data.reviewImageList")
                        .description("리뷰 이미지 목록")
                        .type(JsonFieldType.ARRAY),
                    fieldWithPath("data.reviewImageList[].order")
                        .description("이미지 순서")
                        .type(JsonFieldType.NUMBER),
                    fieldWithPath("data.reviewImageList[].viewUrl")
                        .description("이미지 URL")
                        .type(JsonFieldType.STRING),
  ...
  void review_modify_test() throws Exception {
    ...
    when(reviewService.modifyReview(any(ReviewModifyRequest.class), any(), any()))
        .thenReturn(ReviewResultResponse.response(MODIFY_SUCCESS, reviewId));

    // then
    mockMvc
        .perform(
            MockMvcRequestBuilders.patch("/api/v1/reviews/{reviewId}", reviewId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
  ...
  void review_delete_test() throws Exception {
    ...
    when(reviewService.deleteReview(anyLong(), anyLong()))
        .thenReturn(ReviewResultResponse.response(DELETE_SUCCESS, reviewId));

    // then
    mockMvc
        .perform(
            MockMvcRequestBuilders.delete("/api/v1/reviews/{reviewId}", reviewId)
                .contentType(MediaType.APPLICATION_JSON)
                .with(csrf()))
        .andExpect(status().isOk())
        .andDo(
  ...
  void review_status_chagne() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.patch("/api/v1/reviews/{reviewId}/display", reviewId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    objectMapper.writeValueAsString(
        ...
        .andDo(
            document(
                ...
                requestFields(fieldWithPath("status").type(STRING).description("리뷰 상태")),
                responseFields(
                    fieldWithPath("success").description("응답 성공 여부"),
                    fieldWithPath("code").description("응답 코드(http status code)"),
                    fieldWithPath("data.codeMessage").description("성공 메시지 코드"),
